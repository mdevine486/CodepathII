<?php
  ...
  function validate_state($state, $errors=array()) {
    ...
    if(is_blank($state['code'])){
      $errors[] = h("State code required");
    }else if(!has_length($state['code'], ['min' => 2 , 'max' =>255])){
      $errors[] = h("State code must be between 2 and 255 characters.");
    }  
    return $errors;
  }
  ...
  function validate_territory($territory, $errors=array()) {
    ...
    if(is_blank($territory['position'])){
      $errors[] = h("State position required");
    }else if(!has_valid_number_format($territory['position'])){  // extra validation
      $errors[] = h("State position must be a valid number.");
    } 
    return $errors;
  }
  ...
  function validate_salesperson($salesperson, $errors=array()) {
    ...
    if(is_blank($salesperson['phone'])){
        $errors[] = h("Phone number required.");
      }else if(!has_valid_phoneNumber_format($salesperson['phone'])){
        $errors[] = h("Enter a valid phone number");
      }else if( strlen(preg_replace('/[^0-9]/','', $salesperson['phone'])) != 10){  // extra validation
        $errors[] = h("Phone number must contain 10 digits");
      }

    if(is_blank($salesperson['email'])){
        $errors[] = h("Email required."); 
      }else if(!has_valid_email_format($salesperson['email'])){             // extra validation included
        $errors[] = h("Enter a valid Email address.");
      }
